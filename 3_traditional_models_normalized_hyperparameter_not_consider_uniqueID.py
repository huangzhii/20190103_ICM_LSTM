...
def parse_args():
    ...
    parser.add_argument('--results_dir', default='/home/zhihuan/Documents/20181207_Hypoxemia/20190103_ICM_LSTM/Results/Traditional_20190107_normalized', help="results dir")
...
if __name__=='__main__':
    ...
    mtds = ["logit_l1", "logit_l2", "NN", "NN_l2", "AdaBoost", "GBC", "RFC"]
    hyperparameter_logistic = [0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4]
    hyperparameter_nn_hidden_layer = [16, 32, 64, 128]
    for mtd in mtds:
        print(mtd)
        if mtd == "logit_l1": # around 8 mins for all folds
            ...
            for c in hyperparam_list:
                regr_list.append(LogisticRegression(penalty='l1', C=c))
        if mtd == "logit_l2": # around 4 mins for all folds
            regr_list = []
            hyperparam_list = hyperparameter_logistic
            for c in hyperparam_list:
                regr_list.append(LogisticRegression(penalty='l2', C=c))
        if mtd == "NN": # around 2 mins for all folds
            regr_list = []
            hyperparam_list = hyperparameter_nn_hidden_layer
            for c in hyperparam_list:
        ...
        if mtd == "NN_l2": # around 2 mins for all folds
            ...
            for c in hyperparam_list:
                regr_list.append(MLPClassifier(solver='adam', alpha=1e-5, max_iter=2000, # alpha is L2 reg
                                 hidden_layer_sizes=(c), random_state=1))
        if mtd == "AdaBoost":
            regr_list = [AdaBoostClassifier(n_estimators=100)]
        if mtd == "GBC":
            regr_list = [GradientBoostingClassifier(n_estimators=100)]
        if mtd == "RFC":
            regr_list = [RandomForestClassifier(n_estimators=300)]
